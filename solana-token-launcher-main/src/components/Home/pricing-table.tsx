...
const plans = [
  {
    ...
    button: 'Test Free',
    buttonColor: 'bg-[#0D0D0D] hover:bg-[#1a1a1a] border border-[#1a1a1a]',
  ...
  },
  {
    ...
    button: 'Launch Token',
    buttonColor: 'bg-[#FF2D2D] hover:bg-[#B10000]',
    buttonTextColor: 'text-white',
    popular: true,
    badge: 'Most Popular',
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6">
        ...
        >
          <div className="inline-flex items-center gap-2 bg-[#FF2D2D]/10 border border-[#FF2D2D]/20 rounded-full px-4 py-1.5 mb-6">
            ...
            <span className="text-xs font-semibold text-[#FF2D2D] uppercase tracking-wider">Transparent Pricing</span>
          </div>
          
          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-white mb-4 tracking-tight">
            Simple, <span className="text-[#FF2D2D]">Fair Pricing</span>
          </h2>
        ...
        <div className="grid md:grid-cols-3 gap-6 lg:gap-8">
          {plans.map((plan, index) => (
            <motion.div
              ...
              className={`group relative bg-gradient-to-b from-[#0D0D0D]/80 to-[#0D0D0D]/40 backdrop-blur-sm rounded-2xl p-8 border ${
                plan.popular
                  ? 'border-[#FF2D2D]/50 shadow-2xl shadow-[#FF2D2D]/10 hover:shadow-[#FF2D2D]/20'
                  : 'border-[#1a1a1a] hover:border-[#FF2D2D]/30'
              } transition-all duration-500 hover:-translate-y-1`}
            >
              {/* Popular Badge */}
              {plan.popular && (
                <div className="absolute -top-4 left-1/2 -translate-x-1/2 bg-[#FF2D2D] text-white text-xs font-bold uppercase tracking-wider px-4 py-1.5 rounded-full shadow-lg shadow-[#FF2D2D]/25">
                  {plan.badge}
                </div>
              )}
              ...
              {plan.highlight && !plan.popular && (
                <div className="absolute -top-3 right-4 bg-[#0D0D0D] border border-[#1a1a1a] text-[#BDDBDB] text-[10px] font-bold uppercase tracking-wider px-3 py-1 rounded-full">
                  {plan.highlight}
                </div>
              )}
              ...
              <div className="mb-6">
                ...
                <p className="text-sm text-[#BDDBDB]">{plan.priceNote}</p>
                <p className="text-sm text-[#BDDBDB] mt-3 leading-relaxed">{plan.description}</p>
              </div>
              ...
              <ul className="space-y-3.5">
                {plan.features.map((feature, idx) => (
                  <li key={idx} className="flex items-start gap-3 text-sm">
                    {feature.included ? (
                      <div className="w-5 h-5 rounded-full bg-[#FF2D2D]/10 flex items-center justify-center flex-shrink-0 mt-0.5">
                        <Check className="h-3 w-3 text-[#FF2D2D]" />
                      </div>
                    ) : (
                      <div className="w-5 h-5 rounded-full bg-[#0D0D0D] flex items-center justify-center flex-shrink-0 mt-0.5">
                        <X className="h-3 w-3 text-[#1a1a1a]" />
                      </div>
                    )}
                    <span className={feature.included ? 'text-white' : 'text-[#BDDBDB] opacity-50'}>
                      {feature.name}
                    </span>
                  </li>
                ))}
        ...
        >
          <div className="bg-[#0D0D0D]/50 backdrop-blur-sm rounded-2xl border border-[#1a1a1a] p-6 sm:p-8">
            ...
            <div className="space-y-3">
              {competitorComparison.map((item, idx) => (
                <div key={idx} className="flex items-center justify-between py-2.5 border-b border-[#1a1a1a]/50 last:border-0">
                  <span className="text-sm text-[#BDDBDB]">{item.feature}</span>
                  <div className="flex items-center gap-6">
                    <div className="text-right">
                      <div className="text-xs text-[#BDDBDB] opacity-50 mb-0.5">Competitor</div>
                      <div className="text-sm text-[#BDDBDB] opacity-70">{item.them}</div>
                    </div>
                    <div className="text-right min-w-[80px]">
                      <div className="text-xs text-[#FF2D2D] mb-0.5">ZRP</div>
                      <div className={`text-sm font-semibold ${item.better ? 'text-[#FF2D2D]' : 'text-white'}`}>
                        {item.us}
                        {item.better && <span className="ml-1 text-[#FF2D2D]">✓</span>}
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <p className="text-center text-xs text-[#BDDBDB] opacity-50 mt-6">
        ...
        >
          <p className="text-[#BDDBDB] text-sm flex items-center justify-center gap-2">
            <Info className="h-4 w-4 flex-shrink-0" />
            Each authority revocation adds 0.15 SOL. Network gas fees are covered.
            <Link href="/pricing" className="text-[#FF2D2D] hover:text-[#B10000] font-medium inline-flex items-center gap-1 ml-1 transition-colors">
