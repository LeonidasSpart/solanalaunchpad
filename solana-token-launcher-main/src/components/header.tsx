...
const Header = () => {
  ...
  const mainLinks = [
    ...
    { href: '/launchpad', label: 'Launchpad', highlight: true },
  ];
  ...
  const ecosystemLinks = [
    { href: '/dao', label: 'DAO', icon: <Landmark className="h-4 w-4" /> },
    { href: '/nft', label: 'NFT', icon: <ImageIcon className="h-4 w-4" /> },
    { href: '/nft-staking', label: 'NFT Staking', icon: <Trophy className="h-4 w-4" /> },
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="flex items-center justify-between h-20">
          <Link href="/" className="flex items-center group flex-shrink-0">
            <div className="relative">
              <Image src="/logo.png" alt="ZRP Logo" width={88} height={88} className="rounded-xl" priority />
          ...
          <nav className="hidden lg:flex items-center gap-1">
            {mainLinks.map((link) => (
              <Link key={link.href} href={link.href}
                className={`px-4 py-2 text-sm font-medium rounded-xl transition-all duration-200 relative ${
                  isActive(link.href) ? 'text-white bg-[#FF2D2D]/20' : link.highlight
                    ? 'text-[#FF2D2D] hover:text-white hover:bg-[#FF2D2D]' : 'text-[#BDDBDB] hover:text-white hover:bg-[#1a1a1a]/50'
                }`}>
                {link.label}
                {link.highlight && <span className="absolute -top-1 -right-1 h-2 w-2 bg-[#FF2D2D] rounded-full animate-pulse" />}
              </Link>
            ))}
            ...
            {featureLinks.map((link) => (
              <Link key={link.href} href={link.href}
                className={`flex items-center gap-1.5 px-4 py-2 text-sm font-medium rounded-xl transition-all duration-200 ${
                  isActive(link.href) ? 'text-white bg-[#FF2D2D]/20' : 'text-[#BDDBDB] hover:text-white hover:bg-[#1a1a1a]/50'
                }`}>
            ...
            <div className="relative">
              <button onClick={() => setIsToolsOpen(!isToolsOpen)} onMouseEnter={() => setIsToolsOpen(true)}
                ...
                Tools<ChevronDown className={`h-3.5 w-3.5 transition-transform ${isToolsOpen ? 'rotate-180' : ''}`} />
              </button>
              {isToolsOpen && (
                <div className="absolute top-full mt-2 left-0 w-56 bg-[#0D0D0D] border border-[#FF2D2D]/20 rounded-xl shadow-2xl shadow-[#FF2D2D]/10 overflow-hidden"
                  ...
                  {toolsLinks.map((link) => (
                    <Link key={link.href} href={link.href} onClick={() => setIsToolsOpen(false)}
            ...
            <div className="relative">
              <button onClick={() => setIsEcosystemOpen(!isEcosystemOpen)} onMouseEnter={() => setIsEcosystemOpen(true)}
                className="flex items-center gap-1 px-4 py-2 text-sm font-medium rounded-xl text-[#BDDBDB] hover:text-white hover:bg-[#1a1a1a]/50 transition-all duration-200">
                Ecosystem<ChevronDown className={`h-3.5 w-3.5 transition-transform ${isEcosystemOpen ? 'rotate-180' : ''}`} />
              </button>
              {isEcosystemOpen && (
                <div className="absolute top-full mt-2 left-0 w-56 bg-[#0D0D0D] border border-[#FF2D2D]/20 rounded-xl shadow-2xl shadow-[#FF2D2D]/10 overflow-hidden"
                  onMouseLeave={() => setIsEcosystemOpen(false)}>
                  {ecosystemLinks.map((link) => (
                    <Link key={link.href} href={link.href} onClick={() => setIsEcosystemOpen(false)}
                      className="flex items-center gap-3 px-4 py-3 text-sm text-[#BDDBDB] hover:text-white hover:bg-[#FF2D2D]/10 transition">
                      <span className="text-[#FF2D2D]">{link.icon}</span>{link.label}
                    </Link>
                  ))}
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}
              className="lg:hidden text-[#BDDBDB] hover:text-white p-2 rounded-lg hover:bg-[#1a1a1a]/50 transition" aria-label="Toggle menu">
        ...
        {isMobileMenuOpen && (
          <div className="lg:hidden border-t border-[#FF2D2D]/10 py-4 space-y-2">
            <Link href="/create-mint" onClick={() => setIsMobileMenuOpen(false)}
              ...
              <Rocket className="h-4 w-4" />Create Your Token
            </Link>
            <div className="grid grid-cols-2 gap-2 pt-2">
              <Link href="/tokens" onClick={() => setIsMobileMenuOpen(false)}
                className="text-center px-4 py-2.5 text-sm text-[#BDDBDB] hover:text-white hover:bg-[#1a1a1a]/50 rounded-xl transition">Tokens</Link>
              <Link href="/launchpad" onClick={() => setIsMobileMenuOpen(false)}
                className="text-center px-4 py-2.5 text-sm text-[#FF2D2D] hover:text-white hover:bg-[#FF2D2D]/20 rounded-xl transition">Launchpad</Link>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2 pt-2 border-t border-[#1a1a1a]">
              <Link href="/faq" onClick={() => setIsMobileMenuOpen(false)}
                className="text-center px-4 py-2.5 text-sm text-[#BDDBDB] hover:text-white hover:bg-[#1a1a1a]/50 rounded-xl transition">FAQ</Link>
              <Link href="/about" onClick={() => setIsMobileMenuOpen(false)}
                className="text-center px-4 py-2.5 text-sm text-[#BDDBDB] hover:text-white hover:bg-[#1a1a1a]/50 rounded-xl transition">About</Link>
            </div>
          </div>
        )}
      </div>
