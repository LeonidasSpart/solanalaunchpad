...
export default function AdminNFTStaking({ token }: { token: string | null }) {
  ...
  const fetchPools = async () => {
    ...
    setError(null);
    try {
      const res = await fetch('/api/admin/nft-staking/pools', {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const url = editingPool
      ? `/api/admin/nft-staking/pools/${editingPool.id}`
      : '/api/admin/nft-staking/pools';
  ...
  const handleDelete = async (id: number) => {
    ...
    if (!token) return;
    try {
      const res = await fetch(`/api/admin/nft-staking/pools/${id}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` },
      });
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-xl font-semibold text-white">NFT Staking Pools</h2>
        <button
          onClick={() => { setEditingPool(null); setForm({ collection_mint: '', reward_mint: '', token_symbol: '', token_name: '', apy: 0, lock_duration: 0, min_stake: 1, max_stake: 0, is_active: true }); setShowModal(true); }}
        ...
        <table className="w-full text-sm">
          <thead className="bg-[#1a1a1a]/30">
            <tr>
              ...
              <th className="px-4 py-2 text-left text-[#BDDBDB]">Collection</th>
          ...
          <tbody>
            {pools.length === 0 ? (
              <tr>
                <td colSpan={8} className="text-center py-4 text-[#BDDBDB]">No pools found.</td>
              </tr>
            ) : (
              pools.map((pool) => (
                <tr key={pool.id} className="border-t border-[#1a1a1a]/50 hover:bg-[#1a1a1a]/20">
                  <td className="px-4 py-2 text-white">{pool.id}</td>
                  <td className="px-4 py-2 text-white font-mono text-xs">{pool.collection_mint.slice(0, 8)}...</td>
          ...
          <div className="bg-[#0D0D0D] border border-[#1a1a1a] rounded-xl p-6 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-white text-sm block">Collection Mint *</label>
                  <input
                    type="text"
                    name="collection_mint"
                    value={form.collection_mint || ''}
                ...
                <div>
                  <label className="text-white text-sm block">Lock Duration (seconds)</label>
                  <input
                    type="number"
                    name="lock_duration"
              ...
              <div className="flex gap-3 pt-3">
                <button
                  type="submit"
                  className="flex-1 py-2 bg-[#FF2D2D] hover:bg-[#B10000] text-white rounded-xl transition"
                >
                ...
                <button
                  ...
                  className="flex-1 py-2 bg-[#1a1a1a] hover:bg-[#2a2a2a] text-white rounded-xl transition"
                >
                  Cancel
                </button>
              </div>
